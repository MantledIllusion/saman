...
public class SingleSynchronizationTest {
	...
	@Before
	public void before() {
		this.service = ConversionServiceFactory.of(new BiSynchronizer<SourcePojo, TargetPojo>() {

			@Override
			public TargetPojo fetchTarget(SourcePojo source) throws Exception {
			...
			@Override
			public void toTarget(SourcePojo source, TargetPojo target, ConversionService service) throws Exception {
				target.value = source.value;
			}
			
			@Override
			public TargetPojo persistTarget(TargetPojo target, SourcePojo source) throws Exception {
				if (target.id == null) {
			...
			@Override
			public SourcePojo fetchSource(TargetPojo target) throws Exception {
			...
			@Override
			public void toSource(TargetPojo target, SourcePojo source, ConversionService service) throws Exception {
				source.value = target.value;
			}
			
			@Override
			public SourcePojo persistSource(SourcePojo source, TargetPojo target) throws Exception {
				if (source.id == null) {
	...
	public void testSynchronization() {
		...
		TargetPojo target = this.service.convert(source, TargetPojo.class);
		assertEquals(1, this.mockDb.size());
		assertEquals(VALUE, target.value);
		
		SourcePojo convertedSource = this.service.convert(target, SourcePojo.class);
